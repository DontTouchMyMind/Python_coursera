# Код приложения разбит на классы Storage, StorageDriver и MetricsStorageServerProtocol.
...
# StorageDriver — класс представляющий интерфейс для работы с хранилищем.
# Передача объекта хранилища при инициализации, позволяет абстрагироваться от конкретной
# реализации самого хранилища (мы можем реализовать хранение на файловой системе или на удаленном сервере,
# при этом в код класса StorageDriver не придется вносить изменения).
# В методе __call__ реализована логика разбора входных данных.
# MetricsStorageServerProtocol — класс, который реализует asyncio-сервер.
